        ...
        if constexpr (Container::ops & serialization_options::LN_COMPACTED)
        {
            ...
            else
            {
                ...
                if (byte_size == cur_type_size)
                {
                    ...
                    cont.offset += cur_type_size;
                    value = static_cast<T>((temp >> 4) | static_cast<size_t>(*cont.cur()) << (8 * (cur_type_size - 1) + len_control_bit));
                    cont.offset += 1;
                    return cont;
                }
                ...
                value = static_cast<T>((temp >> len_control_bit) & static_cast<size_t>(-1) >> (8 * (7 - byte_size) + len_control_bit));
    ...
    inline static Container& serialize(Container& cont, const std::vector<T>& value)
    {
        ...
        else
        {
            ...
            if constexpr (std::is_same<T, bool>::value
                          ...
                          || std::is_same<T, float64_t>::value)
            {
                cont.append(value.data(), value.size() * sizeof(T));
                return cont;
            }
            else
            {
...
struct serialization_container
{
    ...
    inline serialization_container& append(const void* data, size_t size) LN_NOEXCEPT
    {
    ...
    inline const uint8_t* cur() const LN_NOEXCEPT
    {
        return reinterpret_cast<const uint8_t*>(cont.data()) + offset;
    }

    inline size_t size() const LN_NOEXCEPT
    {
        return static_cast<size_t>(cont.size());
    }

    template <class T>
    inline serialization_container& operator<<(const T& value) LN_NOEXCEPT
    {
        return serialization_core::serialize(*this, value);
    }

    template <class T, size_t N>
    inline serialization_container& operator<<(const T (&value)[N]) LN_NOEXCEPT
    {
        return serialization_core::serialize(*this, value);
    }

    template <class T>
    inline serialization_container& operator>>(T& value) LN_NOEXCEPT
    {
        return serialization_core::deserialize(*this, value);
    }

    template <class T, size_t N>
    inline serialization_container& operator>>(T (&value)[N]) LN_NOEXCEPT
    {
        return serialization_core::deserialize(*this, value);
    }
};
